...
class BBoxUtility(object):
    ...
    def decode_boxes(self, mbox_loc, mbox_priorbox, variances):
        ...
        """
        # prior: 事前の
        # mbox_priorbox: SSDで読み込んだ画像の一部分の座標？
        prior_width = mbox_priorbox[:, 2] - mbox_priorbox[:, 0]
        prior_height = mbox_priorbox[:, 3] - mbox_priorbox[:, 1]
        prior_center_x = 0.5 * (mbox_priorbox[:, 2] + mbox_priorbox[:, 0])
        prior_center_y = 0.5 * (mbox_priorbox[:, 3] + mbox_priorbox[:, 1])
        # mbox_loc = [minx, miny, maxx, maxyに対応する何か]
        # prior_width or height と variance を掛けることで実座標に戻る？
        ...
        #print(np.exp(mbox_loc[:, 2] * variances[:, 2]))
        #print(decode_bbox_center_x)
        #print(decode_bbox_center_x[0: 20])
